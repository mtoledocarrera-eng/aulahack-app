...
function generateEmbeddingHttps(text) {
...
async function main() {
    console.log("Iniciando ingesta de currículum (HTTPS)...");
    ...
    for (let i = 0; i < CURRICULUM_DATA.length; i++) {
        ...
        if (newStore.some(e => e.id === entry.id)) continue;
...
`.trim();
        ...
        try {
            const embedding = await generateEmbeddingHttps(textToEmbed);
            ...
            console.error(`Error en ${entry.id}:`, error.message);
        }
    }
